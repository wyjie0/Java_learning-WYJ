...
为了便于处理红黑树代码中的边界条件，使用一个哨兵来代表null。对于一棵红黑树，哨兵null是一个与树中普通结点有相同属性的对象。它的color属性为BLACK，其他属性可以设为任意值。如下图所示，所有指向null的指针都用指向哨兵null的指针替换。
...
从某个结点x出发（不含该结点）到达一个叶结点的任意一条简单路径上的黑色结点个数称为该结点的**黑高**，记为bh(x)。一棵有n个内部结点的红黑树的高度至多为2lg(n+1)

####  旋转
搜索树操作insert和delete在含n个关键字的红黑树上，运行花费时间为O(lgn)。由于这两个操作对树做了修改，结果可能会违反红黑树的性质。为了维护这些性质，必须修改树中某些节点的颜色以及指针结构。
...
c. 如果有任何红黑性质被破坏，则至多只有一条被破坏，或是2，或是4。如果性质2被破坏， 其原因为z是根结点且是红结点。如果性质4被破坏， 其原因为z和z.p都是红结点。

我们需要证明在循环的第一次迭代之前循环不变式为真，每次迭代都保持这个循环不变式成立，并且在循环终止时，这个循环不变式会给出一个有用的性质。
我们先从初始化和终止的不变式证明开始。然后，依据细致地考察循环体如何工作，来证明循环在每次迭代中都保持这个循环不变式。同时，还要说明循环的每次迭代会有两种可能的结果：或者指针z沿着树上移，或者执行某些旋转后循环终止。
    ...
    **终止：** 循环终止是因为z.parent是黑色的。（如果z是根结点，那么z.parent是黑色哨兵）这样，树在终止循环时没有违反性质4（因为z必然是红结点，而z.parent为黑结点，则没有违反性质4）。根据循环不变式，唯一可能不成立的是性质2,。在最后的操作恢复了这个性质，所以当rbInsertFixup终止时，所有的红黑性质都成立。
    **保持：** 先考虑z的父结点是z的祖父结点的左孩子（如果是右孩子则操作完全相反）。根据循环不变式的b部分，如果z.parent是根结点，那么z.parent是黑色的，可知z.p.p存在。因为只有在z.parent是红色时才进入一次循环迭代，所以z.p不可能是根结点。
    情况1和情况2、3的区别在于z父亲的兄弟结点（叔结点）的颜色不同。在所有的三种情况中，z的祖父节点都是黑色的，因为z的父结点为红色才进入了循环，并且，在添加z结点前，该树是一棵红黑二叉树，所以z的祖父结点不能为红结点，只能为黑结点。又由于z的父结点是红色的，因此性质4旨在z和z.p之间被破坏了。
    **情况1：z的叔结点y是红色的**
    在这种情况下我们可以通过变换颜色来维持红黑树的性质。这种情况在z.p和y都是红色时发生。因为z.p.p是黑色的，所以将z.p和y都变为黑色，以此解决z和z.p都是红色的问题，将z.p.p变为红色以保持性质5。然后把z.p.p作为新节点z来重复while循环，因为此时的z也有肯能破坏了红黑树的性质。
![image](https://user-images.githubusercontent.com/25001763/69154424-f83ca280-0b1a-11ea-8166-c7ff2873e635.png)
   ...
   **情况3：z的叔结点y是黑色的且z是一个左孩子**
   如果是情况2的情况下，可以用一个左旋来将此情形转变为情况3，此时结点z为左孩子。因为z和z.p都是红色的，所以该旋转对结点的黑高和性质5都无影响。在情况3中，改变某些结点的颜色并做一次右旋，以保持性质5。因为此时z.p是黑色的，所以无需再执行一次while循环。
![image](https://user-images.githubusercontent.com/25001763/69154459-04c0fb00-0b1b-11ea-8367-386369012471.png)

   
   **分析**
   rbInsert的运行时间？由于一棵有n个结点的红黑树的高度为O(lgn)，因此rbInsert要花费O(lgn)时间。在rbInsertFixup中，仅当情况1发生，然后指针沿着树上升2层，while循环才会重复执行。所有while循环可能被执行的总次数为O(lgn)。因此，rbInsert总共花费O(lgn)时间。此外，该程序所做的旋转不超过2次，因为只要执行了情况2或情况3，while循环就结束了。
...
红黑树中的删除操作的特殊之处：
* 由于结点y的颜色可能改变（后续会被赋为z的颜色），yOriginalColor存储了发生改变前的y颜色。如果y以前的颜色是黑色的话，那么删除或者移动y就有可能破坏红黑树的性质。
* 在删除操作中，我们需要用结点x来记录y的原始位置
* 如果y是红色，当y被删除或移动时，红黑性质仍然保持，原因如下：
  1. 树中的黑高没有变化（如果z是黑色，y也会变为黑色）
  2. 不存在两个相邻的红结点。因为y在树中占据了z的位置，再考虑到z的颜色，树中y的新位置不可能有两个相邻的红结点（如果有两个相邻的红结点，那原始的红黑树就不满足红黑性质）。另外，如果y不是z的右孩子，则y的原右孩子x代替y。如果y是红色，则x一定是黑色，因此用x代替y不可能使两个红结点相邻。
